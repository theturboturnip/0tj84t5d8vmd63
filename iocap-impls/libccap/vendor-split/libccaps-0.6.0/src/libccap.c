...
CCapResult read_bits_range(const struct Cap2024_11_Bits bits,
    ...
    libccap_dbg_trace("read_bits_range perms %x n_cavs %d\n", bits.perms, num_cavs);
    ...
    libccap_dbg_trace("read_bits_range initial base %lx len %lx\n", base, length);
    ...
    if (num_cavs > 0) {
        ...
        if (check_lhs > check_rhs) {
            ...
            if (bits.index == 0) {
                ...
                identity_cav1 = true;
                libccap_dbg_trace("read_bits_range cav1    identity\n", base, length);
            } else {
                // This is invalid
            ...
            libccap_dbg_trace("read_bits_range cav1    base %lx len %lx\n", base, length);
        }
        ...
        if (num_cavs > 1) {
            ...
            libccap_dbg_trace("read_bits_range cav2    base %lx len %lx\n", base, length);
        }
    }
