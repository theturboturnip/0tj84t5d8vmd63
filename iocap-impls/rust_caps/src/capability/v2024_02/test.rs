...
use crate::{capability::{v2024::{perms::CapPermsChain, Cap2024}, AddrRange, CapDecodeErr, CapEncDecErr, CapEncodeErr, CapPerms, Capability}, checked_bits::BitInt, crypto::Aes128HandrolledSig, util::{align_to, align_to_exact}};

use super::{Cap2024_02, EncodedInitialRange};
...
const BASE_ADDR: u64 = 0x7aba8fe3_6ac76c00;
...
// new len  = (1 << 20) + (2 << 15)
// new end = (1 << 64) - (1 << 20) - (1 << 15) - (1 << 10) + (1 << 20) + (2 << 15)
//         = (1 << 64)                         - (1 << 10)             + (1 << 15)
// new end > (1 << 64)

#[test]
fn inexact_base_exact_length_pushes_out_64bit() {
    let base: u64 = ((1u128 << 64) - (1 << 20) - (1 << 15) - 1).try_into().unwrap();
    let length: u64 = (1 << 20) + (1 << 15) - 1;
    let expanded_past_end = check_pack_valid(AddrRange::base_len_64(base, length)).unwrap();
    let expected_base: u64 = ((1u128 << 64) - (1 << 20) - (1 << 15) - (1 << 10)).try_into().unwrap();
    ...
    assert!(expected_end_in_64.is_err());
    BitInt::<u128, 65>::check(expected_end).expect("expected_end should be within 65-bits");
}

type Cap = Cap2024_02<Aes128HandrolledSig>;
const SECRET: u128 = 0;
...
fn test_cap_cav2() -> Result<(), CapEncDecErr> {
    ...
    const CAV2_BASE_POW2: u8 = 15 - 9;

    // identity caveat
    let range_x = 0;
    let range_y = 0x1FF + 1; // 9 bits + 1
