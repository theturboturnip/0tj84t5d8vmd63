...
//! The B|C layout has changed to that presented in https://docs.google.com/spreadsheets/d/1iCPURHf2zLnvSDNQREE8upmmwlQl5h2x7yhd1dHRd8w/edit#gid=840091080 - 10-to-64-bit length, with use of implicit one-bit 
...
#[derive(Debug, Clone)]
pub struct Cap2024_02<TSigner: Signaturer> {
    pub sig: u128,
    pub data: u128,
...
impl<TSigner: Signaturer> PartialEq for Cap2024_02<TSigner> {
    fn eq(&self, other: &Self) -> bool {
        self.sig == other.sig && self.data == other.data
    }
}
impl<TSigner: Signaturer> Cap2024 for Cap2024_02<TSigner> {
    ...
    fn new(secret: u128, secret_key_id: u32, base: u64, len: u128, perms: CapPerms) -> Result<Self, CapEncodeErr> {
        let initial_fullyencoded = EncodedInitialRange::pack_base_and_length_exact(&AddrRange::base_len(base, len))?;
        ...
        Ok(Self {
            ...
            data,
            cap_data,
            addr_range: AddrRange::base_len(base, len),
    ...
    fn new_inexact(secret: u128, secret_key_id: u32, base: u64, len: u128, perms: CapPerms) -> Result<Self, CapEncodeErr> {
        let (initial_fullyencoded, addr_range) = EncodedInitialRange::pack_base_and_length(&AddrRange::base_len(base, len))?;
        let cap_data = CapData {
            perms_chain: CapPermsChain::from_root(perms),
            secret_key_id,
            initial: initial_fullyencoded.partial_decode(),
    ...
    fn refine_using_cav1_towards(&self, new_perms: Option<CapPerms>, target: AddrRange) -> Result<Self, CapEncodeErr> {
        ...
        // If this happens to exceed the initial range length, that's not OK.
        // Cap2024_02 doesn't accept such cav1s and we need to return a status that indicates
        // - cav1 has not refined anything
        // - it's impossible to refine this format further
        // for now, panic.
        let selected_pow2 = max_pow2_of_cav1 - selected_size_div;
        if (1 << selected_pow2) > self.addr_range.len() {
            // TODO don't panic
            let initial_range = &self.addr_range;
            panic!("Top 16(?) bits of base and top were 0x{base_top16:03x} and 0x{top_top16:03x}, selected size_div {selected_size_div} for final pow2 {selected_pow2} but this was larger than the initial range {initial_range:x?}");
        }
    ...
    fn refine_using_cav2_towards(&self, new_perms: Option<CapPerms>, target: AddrRange) -> Result<Self, CapEncodeErr> {
        ...
        let (range_x, range_y) = if cav1_pow2 >= 9 {
            // In the normal case the region is split into 512 sub-regions and we take a contiguous set
            let slice_pow2 = cav1_pow2 - 9;

            // cav1_range encompasses base and base + len
            // =>  (base - cav1_range.base())       <  2^cav1_pow2
            // and (base + len - cav1_range.base()) <= 2^cav1_pow2
            // => when shifted down by (cav1_pow2 - 9), they are both <=2^9

            // If we shift bits out of the bottom, the resulting range will be rounded down
            // => check for that and bump up the range manually.
            // This will still be <= 2^9 because the bottom bits of (top_offset) will only be set if it is not a pow2,
            // thus will only ever happen if top_offset is strictly < 2^cav1_pow2
            // thus will only ever happen if (top_offset >> (cav1_pow2 - 9)) is strictly < 2^9
            // so incrementing will only ever push up to exactly 2^9.
            // The rounding down behaviour is what we want for the base.
            let top_offset = (target.base() + (target.len() as u64) - self.addr_range.base()) as u16;
            let mut range_y = (top_offset >> slice_pow2) as u16;
            if (range_y << slice_pow2) < top_offset {
                range_y = range_y + 1;
            }
            assert!(range_y <= (1 << 9));
            ...
            // If cav1_pow2 is already smaller than 2^9, we can go down to the byte level *but* range_x range_y need to be *left* aligned
            ...
            // =>  (base - cav1_range.base())       <  2^cav1_pow2
            // and (base + len - cav1_range.base()) <= 2^cav1_pow2
            // => when shifted left by (9 - cav1_pow2), they are both <=2^9
            (
                ((target.base() - self.addr_range.base()) << (9 - cav1_pow2)) as u16,
                ((target.base() + (target.len() as u64) - self.addr_range.base()) << (9 - cav1_pow2)) as u16
            )
        };

        // TODO should this always refine, even if it's unnecessary?
        // if new_perms.is_none() && range_x == 0 && range_y == 1 << 9 {
        self.with_cav2(new_perms, range_x, range_y)
    }

    /// Take a subrange of the provided range in 512 increments, optionally reducing access permissions
...
impl<TSigner: Signaturer> Capability for Cap2024_02<TSigner> {
    type Secret = u128;
    type Signature = u128;
...
impl EncodedInitialRange {
    ...
    fn pack_base_and_length(r: &AddrRange) -> Result<(Self, AddrRange), CapEncodeErr> {
        ...
        let encoded_base = {
            let encoded_base = r.start >> (64 - base_bits);
            // If we truncated the start...
            let new_start = encoded_base << (64 - base_bits);
        ...
        // Check b_c is 59 bits long
        #[cfg(feature = "std")]
        assert_eq_hex!(b_c & (u64::MAX >> (64-59)), b_c);
        ...
        if !new_range.encompasses(r) {
            panic!("Algorithm failure: computed range {:x?} doesn't encompass requested range {:x?}", new_range, r)
        }
        Ok((s, new_range))
    }
