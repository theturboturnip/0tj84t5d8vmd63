...
- a backend pipeline stage which intends to loop on itself for between 3 and 7 cycles to compute the output (base,top) range for the capability.
...
class CapException(RuntimeError):
    def __init__(self, *args: object) -> None:
        super().__init__(str(type(self)), *args)
    pass
...
def pack_shift_pair(range_x: int, elem_count: int) -> int:
    return bits(range_x, 8, 0) | (bits(elem_count, 9, 0) << 17)

def unpack_shift_pair(x: int) -> Tuple[int,int]:
    return bits(x, 16, 9), bits(x, 34, 26)
...
def two_cav(
    ...
    elem_width_log_2_sub_9 = elem_width_log_2 - 9 # 7-bit sub
    elem_count = elem_count + 1 # 10-bit add
    if elem_width_log_2_sub_9 < 0:
        shift_in = pack_shift_pair(range_x, elem_count) # 65-bit assign
        shift_amt = elem_width_log_2
        elem_width_log_2 = 0 # x-bit assign
    else:
        shift_in = range_x # 65-bit assign
        shift_amt = elem_width_log_2_sub_9
        elem_width_log_2 = elem_width_log_2_sub_9 # 7-bit assign
    ...
    shift_out = shift_in << shift_amt # 65-bit shift of either (elem_count, range_x) or (range_x)
    if elem_width_log_2_sub_9 < 0:
        start_offset, elem_count = unpack_shift_pair(shift_out) # two halves
    else:
        start_offset = shift_out # can't take a half, might go all the way up
...
def eval_add_sub_op(op: Tuple[AddSubOp, int, int]):
...
class CalcState:
    ...
    def calc(self) -> "CalcResults":
        return CalcResults(
            ...
            eval_add_sub_op(self.add_op_65),
            eval_add_sub_op(self.add_op_16),
            eval_add_sub_op(self.add_op_9),
        )
...
def n_cav_together_preptime(
    ...
    if n_caveats == 0:
        ...
        state = CalcState(
            ...
            add_op_9=(AddSubOp.SUB, elem_width_log_2, 9),
        )
        #--------------------------------------------------
        res = state.calc()
        ...
        if n_caveats == 1:
            ...
            elem_width_log_2_sub_9 = res.added_9 # elem_width_log_2 - 9 # 7-bit sub
            if elem_width_log_2_sub_9 < 0:                
                ...
                shl_to_shift = range_x # 65-bit assign
                shl_amt = elem_width_log_2_sub_9
        ...
        if n_caveats == 1:
            ...
            if elem_width_log_2_sub_9 < 0:
                start_offset, elem_count = unpack_shift_pair(res.shifted) # two halves
            else:
                start_offset = res.shifted # shift_out # can't take a half, might go all the way up
...
def check_range(cap, range_x, range_expected):
    if range_x != range_expected:
        raise AssertionError(f"({range_x[0]:x}, {range_x[1]:x} != {range_expected[0]:x}, {range_expected[1]:x})\n{cap}")
...
class Cap2024_02:
    ...
    @staticmethod
    def from_hex(hex: str) -> "Cap2024_02":
        ...
        b_c = cap_bits(5, 63)
        index_size_div = cap_bits(64, 67)
        index = cap_bits(68, 82)
        range_x = cap_bits(83, 91)
        range_y_minus_one = cap_bits(92, 100)
...
class Test:
    ...
    def run_test(self):
        ...
        cap = Cap2024_02.from_hex(self.capability_text)
        ...
        try:
            base, top = n_cav(n_caveats, cap.range_y_minus_one, cap.range_x, cap.index, cap.index_size_div, cap.encoded_elem_width, cap.b_c)
            if self.decode_err is not None:
                raise AssertionError(f"Expected decode error {self.decode_err} from n_cav but got None\n{cap}")
            check_range(cap, (base, top), (self.base, self.top))
        except CapException as ex:
            if self.decode_err is None or type(self.decode_err) != type(ex):
                raise AssertionError(f"Expected decode error {self.decode_err} from n_cav but got {ex}\n{cap}")


        try:
            base, top = n_cav_together(n_caveats, cap.range_y_minus_one, cap.range_x, cap.index, cap.index_size_div, cap.encoded_elem_width, cap.b_c)
            if self.decode_err is not None:
                raise AssertionError(f"Expected decode error {self.decode_err} from n_cav but got None\n{cap}")
            check_range(cap, (base, top), (self.base, self.top))
        except CapException as ex:
        ...
        try:
            base, top = n_cav_together_preptime(n_caveats, cap.range_y_minus_one, cap.range_x, cap.index, cap.index_size_div, cap.encoded_elem_width, cap.b_c)
            if self.decode_err is not None:
                raise AssertionError(f"Expected decode error {self.decode_err} from n_cav but got None\n{cap}")
            check_range(cap, (base, top), (self.base, self.top))
        except CapException as ex:
...
if __name__ == '__main__':
    ...
    if args.check_unpack:
        print(Cap2024_02.from_hex(args.check_unpack))
    else:
        for test in csv_tests(args.testfile):
            test.run_test()